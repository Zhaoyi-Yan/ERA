...
parser.add_argument('--save_prefix_root', default='experiments', type=str,
                    help='Prefix root to save checkpoints')
...
parser.add_argument('--isAccuX', default=0, type=int,
                    help='0 or 1, whether using accuX or common parallel gap model')
parser.add_argument('--featcls', default=0, type=int,
                    help='0 or 1, whether using cls loss on gap model and aligned feature, the feat cls weight is the same as ori_loss_weight')
parser.add_argument('--featKL', default=0, type=int,
                    help='0 or 1, whether using KL loss on gap model and aligned feature, the feat KL weight is the same as kd_loss_weight')
parser.add_argument('--up_dynamic', default=1, type=int,
                    help='whether using learnable up fc, if false, then using interpolation')
parser.add_argument('--gap_model_detach', default=1, type=int,
                    help='usually we need to detach the gradient with respect to the input of gap model')
parser.add_argument('--feat_distill', default=0, type=int,
                    help='whether feat distill, it will be automatically set when kd_method contains krd substring')
parser.add_argument('--feat_distill_weight', default=5, type=float,
                    help='the feature distillation weight before the gap model')
parser.add_argument('--gap_feat_weight', default=1, type=float,
                    help='the feature distillation weight on the outputs of the gap feature')
parser.add_argument('--linear', default='flat', type=str,
                    help='whether linear increasing/decreasing the layer numbers')
parser.add_argument('--delta_x_fn', default='cosine', type=str,
                    help='delta_x_fn')
parser.add_argument('--delta_x_weight', default=1, type=float,
                    help='the weight of delta_x_fn')
parser.add_argument('--merge_alpha', default=0.5, type=float,
                    help='the weight of mimic teacher logit')
parser.add_argument('--gap_n_blocks', default=2, type=int,
                    help='the block numbers of gap model')
parser.add_argument('--gap_n_module', default=2, type=int,
                    help='the number of gap model')
parser.add_argument('--is_multi_fc', default=0, type=int,
                    help='whether using multiple fc for multiple output of gap model')
parser.add_argument('--n_share_blocks', default=0, type=int,
                    help='the share blocks of the gap model, it should be no greater than nBlocks-1')
parser.add_argument('--final_out_type', default='acc', type=str,
                    help='the output type of final output')
parser.add_argument('--scales', default="1,1,1,1", type=str,
                    help='the scales of the training')
parser.add_argument('--gap_type', default='enc', type=str,
                    help='enc, unet, etc')
parser.add_argument('--act_type', default='relu', type=str,
                    help='the act layer for gap model')
parser.add_argument('--spectral_weight', default=0.0, type=float,
                    help='the spectral weight of gap model')
parser.add_argument('--after_pool', default=1, type=int,
                    help='whether using the features after pooling')
parser.add_argument('--eval_alphas', default='0.0, 0.5, 1.0', type=str,
                    help='the evaluation alphas')
parser.add_argument('--output_normed', default=0, type=int,
                    help='whether norm the features of output')
parser.add_argument('--start_eval_epoch', default=100, type=int,
                    help='the epoch where you start to eval')
parser.add_argument('--keep_num', default=15, type=int,
                    help='the most checkpoints to keep')

# try wttm
parser.add_argument('--wttm_r', default=0.7, type=float,
                    help='1/T of the wttm loss')
parser.add_argument('--wttm_beta', default=3.5, type=float,
                    help='weight of the wttm loss')

# vq loss
# maybe for imagenet, n_embed should be larger, and for cifar, n_embed should be smaller
parser.add_argument('--n_embed', default=1024, type=int,
                    help='the quantized number')
parser.add_argument('--embed_dim', default=32, type=int,
                    help='the quantized number')
parser.add_argument('--vq_weight', default=1, type=float,
                    help='the vq loss weight')
...
parser.add_argument('--timm_model_path', type=str, default='',
                    help='path to the timm model, for servers that cannot link to the network')
