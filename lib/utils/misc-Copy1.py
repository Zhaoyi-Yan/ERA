...
class CheckpointManager():
    def __init__(self, model, optimizer=None, ema_model=None, optimizer_gap=None,  gap_model=None, ema_model_gap=None, 
                 up_fc_model=None, ema_model_up_fc=None, optimizer_up_fc=None,  teacher_head=None, \
                 save_dir='', keep_num=5, rank=0, additions={}, eval_alphas=None):
        self.model = model
        ...
        self.optimizer_gap = optimizer_gap
        self.ema_model_gap = ema_model_gap
        self.teacher_head = teacher_head
        
        # additonal
        self.up_fc_model = up_fc_model
        self.ema_model_up_fc = ema_model_up_fc
        self.optimizer_up_fc = optimizer_up_fc
        ...
        if self.rank == 0:
            ...
            if eval_alphas is None:
                ...
                prefix = 'epoch,train_loss,test_loss'
                tmp = []
                num = len(eval_alphas)
                for i in range(num):
                    eval_alpha = eval_alphas[i]
                    tmp.append(f',top1_{eval_alpha}')
                for i in range(num):
                    eval_alpha = eval_alphas[i]
                    tmp.append(f',top5_{eval_alpha}')
                final = prefix + ''.join(tmp)
                self.metrics_fp.write(final+'\n')                    

    # TODO: update this function to save the best model.. Now, it just save the best top1_0.0
    def update(self, epoch, metrics, score_key='top1_0.0'):
        if self.rank == 0:
            # Create the data string
            data_str = f"{epoch},{metrics['train_loss']},{metrics['test_loss']}"

            for k in sorted(metrics.keys()): # sorted, so it will be top1_0.0, top1_0.1, ..., top5_0.0
                if k not in ['epoch', 'train_loss', 'test_loss']:
                    data_str += f",{metrics[k]}"

            # Write the data string to the file
            self.metrics_fp.write(data_str + "\n")
    ...
    def _save(self, save_path, epoch, is_best=False):
        ...
        save_dict = {
            ...
            'gap_model': self.gap_model.module.state_dict() if isinstance(self.gap_model, DDP) else self.gap_model.state_dict(),
            'up_fc_model': self.up_fc_model.module.state_dict() if isinstance(self.up_fc_model, DDP) else self.up_fc_model.state_dict(),
            'ema_model': self.ema_model.state_dict() if self.ema_model else None,
            'ema_model_gap': self.ema_model_gap.state_dict() if self.ema_model_gap else None,
            'ema_model_up_fc': self.ema_model_up_fc.state_dict() if self.ema_model_up_fc else None,
            'optimizer': self.optimizer.state_dict() if self.optimizer else None,
            'optimizer_gap': self.optimizer_gap.state_dict() if self.optimizer_gap else None,
            'optimizer_up_fc': self.optimizer_up_fc.state_dict() if self.optimizer_up_fc else None,
            'teacher_head': self.teacher_head.state_dict() if self.teacher_head else None,
        }
    ...
    def load(self, ckpt_path):
        ...
        # for gap model
        if 'state_dict' in save_dict and 'gap_model' not in save_dict:
            save_dict['gap_model'] = save_dict['state_dict']
        if isinstance(self.gap_model, DDP):
            missing_keys, unexpected_keys = \
                self.gap_model.module.load_state_dict(save_dict['gap_model'], strict=False)
        else:
            missing_keys, unexpected_keys = \
                self.gap_model.load_state_dict(save_dict['gap_model'], strict=False)
        if len(missing_keys) != 0:
            logger.info(f'Missing keys in source state dict: {missing_keys}')
        if len(unexpected_keys) != 0:
            logger.info(f'Unexpected keys in source state dict: {unexpected_keys}')      

        if self.ema_model_gap is not None and 'ema_model_gap' in save_dict:
            self.ema_model_gap.load_state_dict(save_dict['ema_model_gap'])
        if self.optimizer_gap is not None and 'optimizer_gap' in save_dict:
            self.optimizer_gap.load_state_dict(save_dict['optimizer_gap'])
        ...
        if self.teacher_head is not None and 'teacher_head' in save_dict:
            self.teacher_head.load_state_dict(save_dict['teacher_head'])
