...
class GAP_net(nn.Module):
    def __init__(self, n_blocks, in_c, in_c_tec, is_multi_fc=0, act_type='relu', n_module=1, linear='flat'):
        ...
        for i in range(n_module):
            if linear == 'up':
                ...
                cur_block = n_module - i
            else:
                assert 1==2
            blocks = [Block_fc_enc(in_c, act_type=act_type) for _ in range(cur_block-1)] + [Block_fc_enc(in_c, last=True, act_type=act_type)]
