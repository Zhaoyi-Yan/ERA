...
class ResNet(nn.Module):
    ...
    def __init__(self, depth, num_filters, block_name='BasicBlock', num_classes=10, in_c_tec=-1):
        ...
        self.up_fc = nn.Identity() if in_c_tec == -1 else nn.Linear(num_filters[3]* block.expansion, in_c_tec)
    ...
    """
    def get_weight_distill(self, return_fc=False):
        bn1, bn2, bn3, conv1, conv2, conv3 = None, None, None, None, None, None
        if isinstance(self.layer1[0], Bottleneck):
            assert 1==2
            bn1 = self.layer1[-1].bn3
            bn2 = self.layer2[-1].bn3
            ...
            conv1 = self.layer1[-1].conv2
            conv2 = self.layer2[-1].conv2
            conv3 = self.layer3[-1].conv2
            
            if return_fc:
                return [conv1, bn1, conv2, bn2, conv3, bn3, self.fc]
            else:
    ...
    def forward(self, x, is_feat=False, out_up_feat=False):
        x = self.conv1(x)
        x = self.bn1(x)
        x = self.relu(x)  # 32x32
        f0 = x
