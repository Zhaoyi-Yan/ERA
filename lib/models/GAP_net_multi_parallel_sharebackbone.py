...
"""
The mainly proposal lies in the share the backbone of multi-head

say M blocks with N modules.
                                   (M-k) blocks   ----
                                 /                    |
block-block-..-block (totally k) -- ...               | -> N modules
                                 \                    |
                                   (M-k) blocks   ----
...
class GAP_net(nn.Module):
    def __init__(self, n_blocks, in_c, in_c_tec, is_multi_fc=0, act_type='relu', n_module=1, n_share_blocks=0):
        super(GAP_net, self).__init__()
        assert n_share_blocks <= n_blocks - 1, "the number of share_blocks is too big!"
        is_up_fc = 0
        if in_c != in_c_tec:
            is_up_fc = 1
        
        self.is_multi_fc = is_multi_fc

        self.up_fc_list = None
        if is_up_fc:
            self.up_fc_list = nn.ModuleList()
            if is_multi_fc:
                for i in range(n_module):
                    self.up_fc_list.append(nn.Linear(in_c, in_c_tec))
            else:
                self.up_fc_list.append(nn.Linear(in_c, in_c_tec))

        self.pred_res_list = nn.ModuleList()
        self.share_list = []
        
        if n_share_blocks == 0:
            self.share_list.append(nn.Identity())
            for i in range(n_module):
                blocks = [Block_fc_enc(in_c, act_type=act_type) for _ in range(n_blocks-1)] + [Block_fc_enc(in_c, last=True, act_type=act_type)]
                self.pred_res_list.append(nn.Sequential(*blocks))
        else:
            share_blocks = [Block_fc_enc(in_c, act_type=act_type) for _ in range(n_share_blocks)]
            self.share_list.extend(share_blocks)
            self.share_list = nn.Sequential(*self.share_list)
            
            for i in range(n_module):
                suffix_blocks = [Block_fc_enc(in_c, act_type=act_type) for _ in range(n_blocks-n_share_blocks-1)] + [Block_fc_enc(in_c, last=True, act_type=act_type)]
                self.pred_res_list.append(nn.Sequential(*suffix_blocks))
    ...
    def forward(self, x):
        outs = []
        # additional for share list
        x = self.share_list(x)
        for seq in self.pred_res_list:
            outs.append(seq(x))  #  parallel
        
        if self.up_fc_list is not None:
            new_outs = []
            if self.is_multi_fc:
                for up_fc, out in zip(self.up_fc_list, outs):
                    new_outs.append(up_fc(out))
            else:
                for out in outs:
                    new_outs.append(self.up_fc_list[0](out)) # only one fc module
            return new_outs
        
        return outs
    def train_params(self, avoid=True): # should avoid up_fc
        if avoid:
            params = sum([m.numel() for k, m in self.named_parameters() if 'up_fc_list' not in k])
        else:
            params = sum([m.numel() for k, m in self.named_parameters()])
        return params
