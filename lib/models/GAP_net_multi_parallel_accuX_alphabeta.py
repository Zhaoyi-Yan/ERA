...
# TODO: make it competible for in_c and tec are the same channels, maybe using identity?
class GAP_net_accuX(nn.Module):
    def __init__(self, n_blocks, in_c, in_c_tec, is_multi_fc=0, act_type='relu', n_module=1):
        super(GAP_net_accuX, self).__init__()
        is_up_fc = 0
        if in_c != in_c_tec:
            is_up_fc = 1
        ...
        # additional for alpha_beta
        self.alpha_list = nn.ParameterList()
        for i in range(n_module-1):
            self.alpha_list.append(nn.Parameter(torch.ones_like(torch.randn(1, in_c))))

        # for beta, it is 1+2+... + (n-1)
        self.beta_list = nn.ParameterList()
        total_betas = n_module*(n_module-1)//2
        for i in range(total_betas):
            self.beta_list.append(nn.Parameter(torch.zeros_like(torch.randn(1, in_c))))
    ...
    def forward(self, x):
        outs = []  # delta_x1, delta_x2, ..., delta_xn
        i=0
        acc_i=0
        for seq in self.pred_res_list:
            if i == 0:
                ...
                cur_in = []
                for ii in range(i):
                    cur_in.append(self.beta_list[acc_i] * outs[ii])
                    acc_i += 1
                cur_in = torch.sum(torch.stack(cur_in, 0), dim=0)
                outs.append(seq(self.alpha_list[i-1] * x + cur_in))
            i += 1
        ...
        if self.up_fc_list is not None:
            ...
            if self.is_multi_fc:
                ...
                for out in outs:
                    new_outs.append(self.up_fc_list[-1](out)) # only one fc module
            return new_outs
