...
"""
1. Do not directly return delta_x, instead return `pre_fc_feat_tec` and `pre_fc_feat_stu` resp.
2. feature distillation loss is not calculated here
...
class KDLoss():
    ...
    def __init__(self, student, teacher, ori_loss, kd_method='kdt4', student_module='', teacher_module='', ori_loss_weight=1.0, kd_loss_weight=1.0, args=None):
        ...
        elif kd_method.startswith('wttm'):
            self.kd_loss = WTTM(args.wttm_r)
            # replace weight with beta
            self.kd_loss_weight = args.wttm_beta
        else:
    ...
    def __call__(self, x, targets, return_residual=False, return_kd_stuff=False):

        with torch.no_grad():
            feats_tec, logits_tec = self.teacher(x, is_feat=True)
            pre_fc_feat_tec = feats_tec[-1]

        # for student, using out_up_feat=True to enable up_fc, so that feat dim can be competiable for torchvision resnet50-> resnet18
        feats_stu, feats_stu_up, logits = self.student(x, is_feat=True, out_up_feat=True) 
        pre_fc_feat_stu = feats_stu[-1]
        pre_fc_feat_stu_up = feats_stu_up[-1]
        ...
        final_loss = ori_loss + kd_loss

        if return_residual:
            if return_kd_stuff:
                return final_loss, pre_fc_feat_stu, pre_fc_feat_stu_up, pre_fc_feat_tec, (logits, logits_tec, self.kd_loss)
            else:
                return final_loss, pre_fc_feat_stu, pre_fc_feat_stu_up, pre_fc_feat_tec
        else:
            return final_loss
