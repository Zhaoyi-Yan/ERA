...
"""
Add more returning items, only for adding kd_loss for gap model, which seems not useful
...
class KDLoss():
    ...
    def __call__(self, x, targets, return_residual=False, return_tec_logits=False):
        ...
        if return_residual:
            if not return_tec_logits:
                return final_loss, pre_fc_feat_stu, pre_fc_feat_tec - pre_fc_feat_stu
            else:
                return final_loss, pre_fc_feat_stu, pre_fc_feat_tec - pre_fc_feat_stu, logits_tec
            
        else:
            if not return_tec_logits:
                return final_loss
            else:
                return final_loss, logits_tec
