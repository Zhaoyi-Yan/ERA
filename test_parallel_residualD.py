...
# import sys
# sys.path.append('/path/to/your/root/directory')
...
def main():
    ...
    args.exp_dir = os.path.join(args.save_prefix_root, args.experiment)

    if not args.resume:
        print('Maually create the resume path')
        args.resume = os.path.join(args.exp_dir, 'best.pth.tar')
...
def validate(args, epoch, model, loader, loss_fn, model_gap=None, teacher_head=None, log_suffix=''):
    ...
    for eval_alpha in args.eval_alphas:
        for batch_idx, (input, target) in enumerate(loader):
            with torch.no_grad():
                # TODO: optimize it, if using `final` we do not need pre_feats_up, so save one fc execution time
